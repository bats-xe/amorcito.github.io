var settings = {
    ...
    kissParticles: {
        length: 200, // Number of kiss particles
        duration: 3, // Kiss particle duration in seconds
        velocity: 50, // Kiss particle velocity in pixels per second
        size: 20, // Kiss particle size in pixels
    },
};
...
// Clase para las partículas de besos
var KissParticle = (function() {
    function KissParticle() {
        this.position = new Point();
        this.velocity = new Point();
        this.age = 0;
    }
    KissParticle.prototype.initialize = function(x, y) { // Initialize kiss particle
        this.position.x = x;
        this.position.y = y;
        this.velocity.x = (Math.random() - 0.5) * settings.kissParticles.velocity; // Velocidad aleatoria
        this.velocity.y = -settings.kissParticles.velocity; // Sube hacia arriba
        this.age = 0;
    };
    KissParticle.prototype.update = function(deltaTime) { // Update kiss particle
        this.position.x += this.velocity.x * deltaTime;
        this.position.y += this.velocity.y * deltaTime;
        this.age += deltaTime;
    };
    KissParticle.prototype.draw = function(context, image) { // Draw kiss particle
        context.globalAlpha = 1 - this.age / settings.kissParticles.duration;
        context.drawImage(image, this.position.x, this.position.y, settings.kissParticles.size, settings.kissParticles.size);
    };

    return KissParticle;
})();

var KissParticlePool = (function() { // Particle pool class for kisses
    var particles, firstActive = 0, firstFree = 0, duration = settings.kissParticles.duration;

    function KissParticlePool(length) { // Constructor
        particles = new Array(length);
        for (var i = 0; i < particles.length; i++)
            particles[i] = new KissParticle();
    }
    KissParticlePool.prototype.add = function(x, y) { // Add kiss particle
    ...
    KissParticlePool.prototype.update = function(deltaTime) { // Update kiss particles
        var i;
    ...
    KissParticlePool.prototype.draw = function(context, image) { // Draw kiss particles
        if (firstActive < firstFree) {
            for (i = firstActive; i < firstFree; i++) {
                particles[i].draw(context, image);
            }
...
(function(canvas) { // Main function
    var context = canvas.getContext('2d'), particles = new ParticlePool(settings.particles.length), kissParticles = new KissParticlePool(settings.kissParticles.length), particleRate = settings.particles.length / settings.particles.duration, kissParticleRate = settings.kissParticles.length / settings.kissParticles.duration, time;
    ...
    var kissImage = new Image(); // Image for kisses
    kissImage.src = './kiss.png'; // Make sure to have the kiss image in this path
    ...
    function render() { // Render function
        ...
        var kissAmount = kissParticleRate * deltaTime;
        for (var j = 0; j < kissAmount; j++) { // Add kiss particles
            kissParticles.add(Math.random() * canvas.width, canvas.height); // Generate from bottom
        }
